...
adicionarTarefa(){
  ...
  this.http.post('http://localhost:3000/tarefas', this.tarefa).subscribe(
    (response: any) => {
...
buscarTarefasUsuario() {
  this.http.get('http://localhost:3000/tarefas?idUsuario=' + this.usuarioService.obterUsuarioLogado()?.id).subscribe(
    (response: any) => {
    ...
    console.error('Erro ao adicionar tarefa:', error),
    () => {
      const tarefasConcluidas = this.tarefasUsuario.filter(t => t.concluido)
      this.porcentagemTarefas = (tarefasConcluidas.length / this.tarefasUsuario.length) * 100;
...
checkboxChange(tarefa: ITarefa) {
  ...
  this.http.put(`http://localhost:3000/tarefas/${tarefa.id}`, tarefa).subscribe(
      () => {
...
excluirTodasTarefas(tarefa: any){
  this.tarefasUsuario.splice(0, this.tarefasUsuario.length)
...
excluirTarefa(tarefa: any){
  ...
  if (index !== -1)
  this.tarefasUsuario.splice(index, 1);
